...
it('parses prototype: (name: string) => void',  () => {
    const type = Builder.parse_function_type("(name: string) => void");
...
it('parses prototype: (thing, name: string) => void',  () => {
    const type = Builder.parse_function_type("(thing, name: string) => void");
...
it('parses prototype: (a => b) => void',  () => {
    const type = Builder.parse_function_type("(x: a => b) => void");
    assert.equal(type.parameters.length, 1);
