...
Deno.test('parses a declare statement',  () => {
    ...
    assertTrue(Array.isArray(stmts));
    assertEquals(stmts.length, 2);
    assertTrue(stmts[0] instanceof DeclareInstanceStatement);
...
Deno.test('parses an import statement',  () => {
    ...
    assertTrue(stmt instanceof ImportStatement);
...
Deno.test('parses an if statement',  () => {
    ...
    assertTrue(stmt.blocks[0].condition);
    assertEquals(stmt.blocks[0].statements.length, 1);
    assertTrue(stmt.blocks[1].condition);
    assertEquals(stmt.blocks[1].statements.length, 2);
    assertTrue(!stmt.blocks[2].condition);
