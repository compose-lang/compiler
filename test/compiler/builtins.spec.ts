...
it('reads sizeof<boolean>',  () => {
    ...
    const unit = Builder.parse_unit("@Export function boolean_sizeof(): i32 { return sizeof(boolean); }");
...
it('reads alignof<boolean>',  () => {
    const compiler = new Compiler();
    const target = new WasmBufferTarget();
    const unit = Builder.parse_unit("@Export function boolean_alignof(): i32 { return alignof(boolean); }");
    compiler.buildOne(unit, target);
    const runner = Runner.of(target.asWasmSource());
...
it('reads alignof<i32>',  () => {
    const compiler = new Compiler();
    const target = new WasmBufferTarget();
    const unit = Builder.parse_unit("@Export function i32_alignof(): i32 { return alignof(i32); }");
    compiler.buildOne(unit, target);
    const runner = Runner.of(target.asWasmSource());
...
it('reads alignof<i64>',  () => {
    const compiler = new Compiler();
    const target = new WasmBufferTarget();
    const unit = Builder.parse_unit("@Export function i64_alignof(): i32 { return alignof(i64); }");
    compiler.buildOne(unit, target);
    const runner = Runner.of(target.asWasmSource());
